...
String message = (String)request.getAttribute("message");
	...
	if(username==null){
		...
		String id="";
		String user="";String password="";String time="",chara="",email="";
		if(method.equals("upm")){
			id=request.getParameter("id");
			List alist=cb.get1Com("select * from user where id='"+id+"'",6);
			user=alist.get(1).toString();
			password=alist.get(2).toString();
			email=alist.get(3).toString();
			time=alist.get(5).toString();
			chara=alist.get(4).toString();
		}
%>
<body>
  ...
  <tr>
    ...
    <form action="<%=basePath %>AdminServlet?method=<%=method%>&id=<%=id%>" method="post" name="form1">  
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">登录帐号：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <%if(method.equals("upm")){ %><input type="text" name="username" class="text-word" value="<%=user %>" readonly/><%}else{ %>
        <input type="text" class="text-word" name="username" required/><% } %>         
        </td>
      </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">初始密码：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <input type="password" name="password" class="text-word" value="<%=password %>" required/>
        </td>
      </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">系统身份：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for"> 
        <select name="chara">
        <%if(method.equals("upm")){ %><option value="<%=chara%>" >&nbsp;&nbsp;<%=chara%></option><%} %>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">联系邮箱：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <input type="text" class="text-word" name="email" value="<%=email %>" required>
        </td>
      </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
