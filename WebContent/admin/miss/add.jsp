...
String message = (String)request.getAttribute("message");
	...
	if(username==null){
		...
		String id="";String bookname="";String writer="";String pub="";String price="",state="",total="";String time=""; 
		if(method.equals("upmiss")){
			id=request.getParameter("id");
			List alist=cb.get1Com("select * from missbook where id='"+id+"'",8);
			...
			total=alist.get(5).toString();
			time=alist.get(7).toString();
		}
%>
<body>
  ...
  <tr>
    ...
    <input type="hidden" name="method" value="<%=method%>" /><input type="hidden" name="id" value="<%=id%>" />
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">在册教材：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <select onClick="javascript:{document.all.writer.value=this.options[this.selectedIndex].getAttribute('writer');document.all.public.value=this.options[this.selectedIndex].getAttribute('pub');document.all.bookname.value=this.options[this.selectedIndex].value;document.all.price.value=this.options[this.selectedIndex].getAttribute('price');}" required>
        <%if(method.equals("upout")){ %>
    		  <option value=<%=bookname %>><%=bookname %></option><%}
        	List listlib=cb.getCom("select * from bookdepot order by id desc", 6);
        	for(int i=0;i<listlib.size();i++){
        		ArrayList list1=(ArrayList)listlib.get(i);
        %>
    		  <option value="<%=list1.get(1).toString() %>" writer="<%=list1.get(2).toString()%>" pub="<%=list1.get(3).toString()%>" price="<%=list1.get(4).toString()%>"><%=list1.get(1).toString() %></option>
    	<%} %>
        </select>  
        </td>
        </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="left" valign="middle" class="borderright borderbottom main-for"> 
        <input type="text" name="bookname" class="text-word" value="<%=bookname %>" required/>    
        </td>
      </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
